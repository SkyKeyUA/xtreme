(() => {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        let coachesTitle = document.querySelectorAll(".coach__title");
        let coachesPosition = document.querySelectorAll(".block-coaches__position");
        let coachesSelect = document.querySelector(".requestform__select");
        let coachesFull = [];
        coachesSelect.innerHTML = "";
        let coachNames = Array.from(coachesTitle).map((obj => obj.textContent.trim()));
        let coachPosition = Array.from(coachesPosition).map((obj => obj.textContent.trim()));
        for (let i = 0; i < coachesTitle.length; i++) coachesFull.push(`${coachPosition[i]} ${coachNames[i]}`);
        for (let i = 0; i < coachesFull.length; i++) {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        const firstBody = document.querySelector(".requestform__body-1");
        const secondBody = document.querySelector(".requestform__body-2");
        const thirdBody = document.querySelector(".requestform__body-3");
        nextButton.addEventListener("click", (function(e) {
            e.preventDefault();
            let firstError = firstFormValidate(form);
            if (firstError === 0) {
                firstBody.classList.add("_active");
                secondBody.classList.add("_active");
            }
        }));
        backButton.addEventListener("click", (function(e) {
            e.preventDefault();
            firstBody.classList.remove("_active");
            secondBody.classList.remove("_active");
        }));
        ...
        async function formSend(e) {
            ...
            if (error === 0) {
                ...
                let response = await fetch("wp-content/themes/elkmoving/assets/files/sendmail/sendmail.php", {
                    method: "POST",
                    body: formData
                });
                secondBody.classList.remove("_active");
                thirdBody.classList.add("_active");
                if (response.ok) {
        ...
        function firstFormValidate(form) {
            let firstError = 0;
            let formReq = document.querySelectorAll("._reqfirst");
            ...
            if (firstError !== 0) alert("Fill in the required fields");
            return firstError;
        }
